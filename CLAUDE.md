...
**Auth**: every request must carry `Authorization: Bearer nf_…`. Tokens live in the `mcp_tokens` table (id, userId FK→users.id ON DELETE CASCADE, name, **tokenHash** (SHA-256 hex, UNIQUE INDEX), **prefix** (first 11 chars — display only), lastUsedAt, createdAt, revokedAt). Generation + lookup helpers in [server/utils/mcpAuth.ts](server/utils/mcpAuth.ts):
...
**Frontend install flow**: [components/McpTokensDialog.vue](components/McpTokensDialog.vue) (triggered from a plug-icon button in the sidebar footer next to the theme toggle) lists active tokens, generates new ones, shows the clear token + a copy-pasteable Claude Desktop config snippet ONCE, and revokes via `useDialog().confirm`. Store: [stores/mcpTokens.ts](stores/mcpTokens.ts), optimistic + rollback like the other stores.
