...
⚠️ **`data/noteforge.db` on this host IS the live production database.** This box runs the prod container itself (`docker compose`, `./data` bind-mounted to `/app/data`, published on `127.0.0.1:3030` behind the reverse proxy) — the file holds real user data, not a dev scaffold. Read-only inspection is fine (`sqlite3 -readonly`, or `docker exec noteforge …`); never write to it, and back it up before anything schema-affecting. Beware that `.env` sets `DATABASE_URL=/app/data/noteforge.db` — the **container** path — so host-side `pnpm dev` / `pnpm db:migrate` do NOT hit this file; export `DATABASE_URL=data/noteforge.db` for a local run, which is also what keeps dev off prod. Note also that user content columns are encrypted at rest under the per-user DEK — only embeddings (`summary_embedding`, `embedding_blob`) and the FTS5 mirror are cleartext, so even with the file in hand you can't read titles/tags/text without the user's key; reason from the schema/code, or ask the user to check in the app.
...
- [server/utils/move.ts](server/utils/move.ts) — document **and folder** relocation, including **across workspaces**. `moveUserFolder` moves a folder with its whole subtree (subfolders, notes, trashed rows included) — ids don't change, so inner `parent_id` / `folder_id` links survive untouched and only the moved root re-parents. ⚠️ `POST /api/documents/bulk` expands a selected folder into its notes for every action EXCEPT `move`, where that expansion would flatten the hierarchy into the destination; `move` therefore skips the expansion and relocates folders as folders, and drops selected notes already covered by a selected subtree. Same-workspace = plain `folderId` change (delegates to `updateUserDocument` with the resolved *workspace* key, not the raw DEK). Cross-workspace = re-key: `documents` + `document_versions` + `doc_analyses` (per-string on the JSON columns) + `doc_chunks.text` are decrypted with the source workspace key and re-encrypted under the destination one, in a single raw-sqlite transaction (same shape as `reencryptWorkspaceContent` in `workspace-share.ts`). Embeddings + `doc_chunks_fts` are untouched — the text didn't change. Two consequences are reported back, not silently absorbed: `doc_links` straddling the two workspaces are dropped (the table is workspace-scoped), and active `share_tokens` are **revoked** because their `wrapped_key` wraps the old key under a raw token we only hold the hash of. Attachments keep their original `workspace_id`.
...
- Deployment is `docker compose up -d --build` on the host itself (no CI/CD, no registry): the image is built from the working tree, so **push first, then rebuild**, or prod ships uncommitted code. `docker/entrypoint.sh` applies pending migrations before Nitro starts (idempotent), and the `./data` bind-mount survives the rebuild — an image rebuild alone never touches the DB.
