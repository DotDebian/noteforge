...
const { t } = useLocale()
useHead(() => ({ title: t('auth.head.signIn') }))
...
async function onSubmit() {
  ...
  try {
    ...
    if (res.recoveryKey) {
      // Legacy account just migrated to encryption — hold here so the user
      // saves the freshly-minted recovery key.
    ...
    error.value = data?.message ?? (e as Error).message ?? t('auth.signIn.errorGeneric')
  } finally {
    pending.value = false
  }
}
...
<template>
  <section v-if="recoveryKey" class="auth-form recovery">
    ...
    <p class="lede">
      Tes notes sont désormais chiffrées avec ton mot de passe. Cette clé est
      la seule façon de retrouver tes données si tu l'oublies. Elle ne sera
  ...
  <form v-else class="auth-form" @submit.prevent="onSubmit" novalidate>
    <p class="eyebrow">{{ t('auth.signIn.eyebrow') }}</p>
    ...
    <div class="actions">
      <button type="submit" class="primary" :disabled="pending">
        <span v-if="!pending">{{ t('auth.signIn.continue') }}</span>
        <span v-else>{{ t('auth.signIn.signing') }}</span>
      </button>
    </div>

    <p class="alt">
      <span>{{ t('auth.signIn.altQuestion') }}</span>
      <NuxtLink to="/register">{{ t('auth.signIn.altLink') }}</NuxtLink>
    </p>
    <p class="alt forgot">
      <NuxtLink to="/recover">Mot de passe oublié&nbsp;?</NuxtLink>
    </p>
  </form>
</template>
...
<style scoped>
.auth-form {
  @apply w-full max-w-[360px] mx-auto;
}
...
.field input::placeholder {
  @apply text-ink-300 dark:text-ink-600;
}
.field input:focus {
  @apply border-ink-900;
}
